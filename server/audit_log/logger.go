...
type Logger interface {
	Create(echo.Context, models.AuditLogType, *uuid.UUID, error) error
	CreateWithConnection(*pop.Connection, echo.Context, models.AuditLogType, *uuid.UUID, error) error
}
...
func (l *logger) Create(context echo.Context, auditLogType models.AuditLogType, user *uuid.UUID, logError error) error {
	return l.CreateWithConnection(l.persister.GetConnection(), context, auditLogType, user, logError)
}

func (l *logger) CreateWithConnection(tx *pop.Connection, context echo.Context, auditLogType models.AuditLogType, user *uuid.UUID, logError error) error {
	if l.storageEnabled {
...
func (l *logger) store(tx *pop.Connection, context echo.Context, auditLogType models.AuditLogType, user *uuid.UUID, logError error) error {
	id, err := uuid.NewV4()
	if err != nil {
...
func (l *logger) logToConsole(context echo.Context, auditLogType models.AuditLogType, user *uuid.UUID, logError error) {
	now := time.Now()
